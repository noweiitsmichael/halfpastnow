class MobileController < ApplicationController
  ...
  def friendEvents
    ...
    query ="SELECT DISTINCT ON (events.id,acts.id) occurrences.end AS end, events.cover_image_url AS cover, venues.phonenumber AS phone, venues.id AS v_id, 
            ...
            WHERE #{where_clause} AND bookmarks.bookmarked_type='Occurrence' AND bookmark_lists.main_bookmarks_list IS TRUE AND occurrences.recurrence_id IS NOT NULL 
            ...
            INNER JOIN bookmark_lists ON bookmarks.bookmark_list_id = bookmark_lists.id 
            INNER JOIN users ON bookmark_lists.user_id = users.id 
            WHERE #{where_clause} AND bookmarks.bookmarked_type='Occurrence' AND bookmark_lists.main_bookmarks_list IS TRUE AND occurrences.recurrence_id IS NULL"
    queryResult = ActiveRecord::Base.connection.select_all(query)
  ...
  def friendEvents1
    tmp ="0"
    @ids = params[:ids].to_s.empty? ? nil : params[:ids].split(',')
    @users = User.find_all_by_uid(@ids)
    


    @users.each{
      |u|
      @occurrencesid = u.bookmarked_events.collect{|o| o.id}
      if @occurrencesid.size > 0
          ...
          @eventIDs.each{ |id|
            ...
            item = {
                      ...
                      :start => s["occurrence_start"] , #20
                      :end => s["end"], #21
                      :name => u.firstname+' '+u.lastname, # 22
                      :id => u.uid #23
                    }
            esinfo << item
