...
private
  ...
  def data
    venues.map do |venue|
      [
        link_to(venue["name"], "/venues/edit/" + venue["id"]),
        h(venue["address"]),
        h(venue["views"]),
  ...
  def fetch_venues
    ...
    if ((sort_column == "name") || (sort_column == "address") || (sort_column == "views"))
      venues_with_existing_events_query += " ORDER BY " + sort_column + " " + sort_direction
    ...
    venues = ActiveRecord::Base.connection.select_all(venues_with_existing_events_query)
